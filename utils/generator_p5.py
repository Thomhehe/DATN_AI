...
- Precondition Handling: ONLY IF the current feature is NOT 'login' AND a test case explicitly requires a LOGIN precondition AND the setup login test case is provided below, then:
  ...
  5. `perform_actions()` MUST continue from the setup's final state without reloading the page.
  6. DO NOT apply this extraction or generate setup methods/flags for non-login preconditions.
...
- INSIDE THE LOOP, generate ONLY the validation checks REQUIRED BY THE TEST CASES. Initialize `found = ""` at the start of the loop.
  + URL Check: ONLY generate `if exp.startswith("http"):`. MUST `try:` wait for URL (15s timeout) before capturing. Playwright: `self.page.wait_for_url("**" + exp.split("://")[-1] + "**", timeout=15000)`. Selenium: `WebDriverWait`. If found, `continue` to next `exp`.
  + HTML5 Validation: ONLY generate `validationMessage` check if a testcase's LOCATOR explicitly specifies "html5". You MUST ensure it's truly triggered by checking `!el.validity.valid` and `el === document.activeElement`. Then, MUST call `self.driver.execute_script("arguments[0].reportValidity();", el)` to force the browser to visually display the tooltip, then append the message. If found, set `found = msg` but DO NOT `continue`.
  + Explicit Locators: Collect ONLY the validation locators you actually defined in `__init__` into a list and check them with a 10000ms wait. Playwright: `el = self.page.locator(loc).first; el.wait_for(state="attached", timeout=10000); txt = el.text_content() or ""; if exp in txt: found = txt.strip(); break`. MUST use `if exp in txt:` for condition! DO NOT reference attributes you did not explicitly define. If `found` is set, DO NOT `continue`.
  + Body Text Fallback: If `found` is still empty AND the test case lacks a specific message locator (and is NOT an html5 case), MUST use Playwright expect to wait for text. Playwright: `from playwright.sync_api import expect` at top, then `try: expect(self.page.locator("body")).to_contain_text(exp, timeout=10000); found = exp except: pass`. Selenium: `try: WebDriverWait(self.driver, 10).until(expected_conditions.presence_of_element_located((By.XPATH, f"//*[contains(., '{{exp}}')]"))); found = exp except: pass`. DO NOT generate this fallback if all test cases have explicit locators.
...
- Test Flow: Init Page -> In test file: `if d.get("requires_login") and hasattr(pom_instance, "setup_login"): pom_instance.setup_login()` else `driver.goto(d.get("url", ...))` (Playwright) / `driver.get(...)` (Selenium). Do NOT call `driver.get()` if `requires_login` is True, as it will reload the page and destroy the session. -> perform_actions -> result = get_result -> assert. The assertion must handle lists: `if isinstance(d["expected"], list): assert any(e in r and r != "" for e, r in zip(d["expected"], result))` else `assert d["expected"] in result or result == d["expected"]`. DO NOT use `if d["id"] == ...` blocks in the test file to branch logic. The test function must be purely data-driven.
